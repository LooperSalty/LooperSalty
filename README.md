# Salut, moi c'est Paul 👋

Développeur passionné par le **game dev**, le **machine learning** et les **expériences 3D interactives**.  
Je construis des moteurs, des agents IA et des mondes procéduraux.

---

## 🔭 Projets notables

| Projet | Stack | Description |
|--------|-------|-------------|
| [**Minecraft**](https://github.com/LooperSalty/Minecraft) | C++17 · OpenGL 3.3 | Clone Minecraft 1.8.9 — rendu voxel, génération de terrain, physique |
| [**nexagen**](https://github.com/LooperSalty/nexagen) | TypeScript · Three.js | Moteur de monde procédural alimenté par l'IA — jouable dans le navigateur |
| [**claw-code**](https://github.com/LooperSalty/claw-code) | Rust | Harness tools haute performance pour agents IA |
| [**claude-orchestra**](https://github.com/LooperSalty/claude-orchestra) | TypeScript · Tauri | Orchestration multi-agents avec interface desktop |

---

## ⚡ Stack

### Langages
<p>
  <img src="https://skillicons.dev/icons?i=ts,js,python,cpp,c,rust,cs,java,php" />
</p>

### Frontend
<p>
  <img src="https://skillicons.dev/icons?i=react,nextjs,vue,html,css,tailwind,vite" />
</p>

### Backend & Data
<p>
  <img src="https://skillicons.dev/icons?i=nodejs,fastapi,laravel,symfony,mongodb,mysql,postgres" />
</p>

### ML / IA
<p>
  <img src="https://skillicons.dev/icons?i=pytorch,tensorflow,anaconda" />
  <a href="https://claude.ai"><img src="./icons/claude.svg" width="48" height="48" alt="Claude" /></a>
  <a href="https://gemini.google.com"><img src="./icons/gemini.svg" width="48" height="48" alt="Gemini" /></a>
</p>

### Game Dev & 3D
<p>
  <img src="https://skillicons.dev/icons?i=unity,unreal,bevy,threejs,blender" />
</p>

### Mobile
<p>
  <img src="https://skillicons.dev/icons?i=flutter,androidstudio" />
</p>

### Outils & DevOps
<p>
  <img src="https://skillicons.dev/icons?i=git,github,linux,docker,tauri,vscode,figma,arch,hyprland" />
</p>

---

## 📊 Stats

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=LooperSalty&theme=tokyonight&hide_border=true" height="165" />
</p>

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=LooperSalty&theme=tokyonight" width="100%" />
</p>

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=LooperSalty&theme=tokyonight" height="165" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=LooperSalty&theme=tokyonight" height="165" />
</p>

---

## 📫 Contact

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Paul_RUIZ-0A66C2?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/paul-ruiz-851465259/)
[![Email](https://img.shields.io/badge/Email-paulruiz944%40gmail.com-EA4335?style=for-the-badge&logo=gmail)](mailto:paulruiz944@gmail.com)

---

<p align="center">
  <i>« Le meilleur code est celui qu'on n'a pas besoin d'écrire. »</i>
</p>
